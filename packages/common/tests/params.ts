...
tape('[Common]: Parameter access for param(), paramByHardfork()', function (
  t: tape.Test
) {
  ...
  t.test('Error cases for param(), paramByHardfork()', function (
    st: tape.Test
  ) {
    ...
    msg =
      'Should throw when calling paramByBlock() with an unsupported hardfork'
    st.throws(f, /supportedHardforks$/, msg)
  ...
  t.test('Parameter updates', function (st: tape.Test) {
    ...
    st.equal(
      c.paramByHardfork('gasPrices', 'netSstoreNoopGas', 'petersburg'),
      null,
      msg
    )
  ...
  t.test('Access by block number, paramByBlock()', function (st: tape.Test) {
    ...
    let msg =
      'Should correctly translate block numbers into HF states (updated value)'
    st.equal(
      c.paramByBlock('pow', 'minerReward', 4370000),
      '3000000000000000000',
      msg
    )
    ...
    st.equal(
      c.paramByBlock('pow', 'minerReward', 4369999),
      '5000000000000000000',
      msg
    )
